...
const fieldLabels = {
  ...
  material1Title: 'Titolo materiale 1',
  material1Type: 'Tipo materiale 1',
  material1Url: 'File / link materiale 1',
  material2Title: 'Titolo materiale 2',
  material2Type: 'Tipo materiale 2',
  material2Url: 'File / link materiale 2',
  material3Title: 'Titolo materiale 3',
  material3Type: 'Tipo materiale 3',
  material3Url: 'File / link materiale 3',
...
function cardToDb(card, userId) {
  return {
    ...
    materials: card.materials || buildMaterialsFromFields(card),
    name: card.name || '',
    claim: card.claim || '',
...
function visibleMaterials(card) {
...
function MaterialUploadField({ index, card, updateCard, session }) {
  const [uploading, setUploading] = useState(false)
  const titleKey = `material${index}Title`
  const typeKey = `material${index}Type`
  const urlKey = `material${index}Url`
  ...
  const uploadFile = async (file) => {
    ...
    const safeFileName = file.name
      ...
      .replace(/[^a-zA-Z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '')
      .toLowerCase() || `materiale-${index}`
    const path = `${session.user.id}/materials/${safeFileName}-${Date.now()}.${ext}`
    ...
    if (error) {
      alert(`Errore upload materiale: ${error.message}`)
      setUploading(false)
      return
    }

    const { data } = supabase.storage.from('card-materials').getPublicUrl(path)
  ...
  return (
    <div className="material-upload-card">
      <div className="material-upload-head">
        <span>Materiale {index}</span>
        <strong>{card[titleKey] || 'Nuovo materiale'}</strong>
      </div>

      <div className="material-upload-grid">
        <label>
          Titolo
          <input value={card[titleKey] || ''} onChange={(event) => updateCard({ [titleKey]: event.target.value })} placeholder="Es. Brochure aziendale" />
        </label>

        <label>
          Tipo
          <select value={card[typeKey] || ''} onChange={(event) => updateCard({ [typeKey]: event.target.value })}>
            <option value="">Seleziona tipo</option>
            {materialTypeOptions.map(option => <option key={option} value={option}>{option}</option>)}
          </select>
        </label>

        <label className="material-url-field">
          Link / file
          <input value={card[urlKey] || ''} onChange={(event) => updateCard({ [urlKey]: event.target.value })} placeholder="Carica un PDF o incolla un link" />
        </label>
      </div>

      <div className="material-actions">
        <label className="btn light" htmlFor={`material-file-${index}`}>{uploading ? 'Caricamento…' : 'Carica PDF / file'}</label>
        <input id={`material-file-${index}`} type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.jpg,.jpeg,.png" onChange={(event) => uploadFile(event.target.files?.[0])} />
        {card[urlKey] && <a className="btn dark" href={normalizeMaterialUrl(card[urlKey])} target="_blank" rel="noreferrer">Apri</a>}
        {card[urlKey] && <button type="button" className="btn light" onClick={() => updateCard({ [urlKey]: '', [titleKey]: '', [typeKey]: '' })}>Rimuovi</button>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="main-grid">
      <section className="page-panel">
        ...
        <EditorSection title="Materiali / Brochure">
          <div className="materials-editor-panel">
